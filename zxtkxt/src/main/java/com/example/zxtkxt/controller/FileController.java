...
//    }
    ...
    @PostMapping("/delete")
    public ApiRest<String> deleteFile(@RequestParam("filePath") String filePath) {
        if (StrUtil.isBlank(filePath)) {
            return success("路径为空，无需删除");
        }
        try {
            // 获取应用程序的工作目录
            String workingDir = System.getProperty("user.dir");
            // 构建文件的绝对路径
            // 注意：前端传来的路径通常是 /images/xxx.jpg，需要去掉开头的 /images/
            // 或者直接拼接到 public 目录下
            String relativePath = filePath.startsWith("/") ? filePath.substring(1) : filePath;
            String absolutePath = workingDir + "/zxtkxt_vue/public/" + relativePath;
            ...
            if (file.exists() && file.isFile()) {
                ...
                return success("文件不存在，无需删除");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return failure("删除出错: " + e.getMessage());
        }
    }
