  ...
  <div class="login-container">
    <div class="login-card">
      <div class="login-header">
        <img src="@/assets/logo.png" alt="Logo" class="login-logo" v-if="hasLogo" @error="hasLogo = false">
        <h2 class="login-title">在线题库系统</h2>
      </div>
      ...
      >
        ...
        <el-form-item>
          <el-button type="primary" @click="submitForm('ruleForm')" class="login-button" :loading="loading">登 录</el-button>
        </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.loading = true;
          // Optimistically set cookie for UX, though usually done after success
...
.login-button {
  width: 100%;
  padding: 22px 0; /* Increase height */
  font-size: 16px;
  letter-spacing: 2px;
  font-weight: bold;
  margin-top: 10px;
